...
class pretrained_model:
    def __init__(self):
        ...
        self.qwen_model, self.qwen_processor = models.qwen_vl()
        # self.llama_model, self.llama_processor = models.llama_vision()
        # self.gemma_model, self.gemma_processor = models.gemma()
        # self.phi4_model, self.phi4_processor, self.phi4_generation_config = models.phi4()
...
def gemma_collate_fn(batch, processor):
    ...
    texts = []
    for q in questions:
        # Gemma 모델은 이미지 플레이스홀더와 함께 텍스트를 처리합니다.
        messages = [
...
def phi4_collate_fn(batch, processor):
    """
    Phi-3-Vision-128k-Instruct 전용.
    apply_chat_template을 사용하여 프롬프트를 구성합니다.
    """
    questions, answers, pil_images = [], [], []
    for q, a, img in batch:
        questions.append(q)
...
def main():
    ...
    # low_phi4_loader = DataLoader(
    #     low_dataset,
    #     batch_size=4,
    #     shuffle=False,
    #     num_workers=4,
    #     collate_fn=partial(phi4_collate_fn, processor=vlm.phi4_processor),
    # )
    ...
    # for batch_inputs, questions, answers in tqdm(low_phi4_loader, desc = "Phi4 Inference"):
    #     batch_inputs = {k: v.to(vlm.phi4_model.device) if hasattr(v, "to") else v
    #                     for k, v in batch_inputs.items()}
        
    #     with torch.no_grad():
    #         gen_ids = vlm.phi4_model.generate(**batch_inputs, generation_config=vlm.phi4_generation_config)

    #     print(vlm.phi4_processor.batch_decode(gen_ids, skip_special_tokens=True))
    #     break
